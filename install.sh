...
for collection in $PACKAGE_COLLECTIONS; do
    for x in ${!collection}; do
        ...
        if [ $collection == "PLASMA_NEXT" ]; then
            cmake .. -DCMAKE_INSTALL_PREFIX=/usr -DSYSCONF_INSTALL_DIR:STRING=/etc -DBIN_INSTALL_DIR:STRING=bin -DDATA_INSTALL_DIR:STRING=share/kde5 -DLIBEXEC_INSTALL_DIR:STRING=libexec/kde5
        else
